...
pub fn Stories() -> impl IntoView {
    ...
    let hide_more_link =
        move || stories.get().unwrap_or(None).unwrap_or_default().len() < 28 || pending.get();
                    ...
                    {move || {
                        if page() > 1 {
                            view! {
                                <a
                                    class="page-link"
                                    href=move || format!("/stories/{}?page={}", story_type(), page() - 1)
        ...
        <li class="news-item">
            ...
            <span class="title">
                {if !story.url.starts_with("item?id=") {
                    ...
                    view! { <A href=format!("/stories/{}", story.id)>{title.clone()}</A> }
                        .into_view()
                }}
                    ...
                    view! {
                        <span>
                            ...
                            {story
                                .user
                                .map(|user| {
                                    view! { <A href=format!("/users/{user}")>{user.clone()}</A> }
